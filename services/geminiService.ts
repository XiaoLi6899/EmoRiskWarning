...
export const generateStudentReport = async (
  ...
  const prompt = `
    ...
    请严格按照以下四个维度，输出最核心的【关键词/短语】(每项3-6个)，用 " | " 分隔。不要使用整句，不要解释。

    1. **核心特征与行为模式**: 如：安全型依恋 | 手机依赖 | 目标清晰
    2. **心理健康情况评估**: 如：焦虑倾向 | 睡眠良好 | 情绪平稳
    3. **发展潜力评估**: 如：领导力潜质 | 逻辑思维强 | 创新能力
    4. **支持策略与危机预警应对**: 如：定期谈话 | 增加运动 | 关注睡眠
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        responseSchema: {
          ...
          required: ["coreCharacteristics", "mentalHealthAssessment", "developmentPotential", "supportStrategies"]
        }
      }
    });
